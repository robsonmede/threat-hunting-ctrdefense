...
SECRETS_EXAMPLE_PATH = STREAMLIT_DIR / "secrets.example.toml"


# Conteúdo do arquivo de exemplo
SECRETS_EXAMPLE_CONTENT = """[auth.users.admin]
...
def backup_file(path: Path) -> None:
    """Cria uma cópia de segurança antes de alterar um arquivo."""
    if not path.exists():
        return
...
def write_file(path: Path, content: str) -> None:
    """Grava um arquivo, criando backup quando ele já existir."""
...
AUTH_CONTENT = r'''from __future__ import annotations
...
CSS_CONTENT = r'''from __future__ import annotations
...
APP_CONTENT = r'''from __future__ import annotations
...
def update_gitignore() -> None:
    if GITIGNORE_FILE.exists():
        lines = GITIGNORE_FILE.read_text(
            encoding="utf-8"
        ).splitlines()
    else:
        lines = []
...
def remove_fixed_ip_links() -> None:
    ...
    # Remove URLs fixas contendo IPs ou hosts HTTP.
    content = re.sub(
        r"https?://[^\s\"'<>]+",
        "",
        content,
    ...
    # Remove links HTML diretos para páginas.
    content = re.sub(
        r"<a[^>]*pages/[^>]*>.*?</a>",
        "",
        content,
    ...
    print("[OK] Links fixos removidos de core/sidebar.py.")
...
def move_invalid_pages() -> None:
    ...
    backup_pages_dir = PAGES_DIR / "_backup_pages"
    moved_count = 0
    ...
    for item in PAGES_DIR.iterdir():
        ...
        if item.name in valid_page_names:
            continue

        backup_pages_dir.mkdir(parents=True, exist_ok=True)

        destination = backup_pages_dir / item.name

        if destination.exists():
            destination = (
                backup_pages_dir
                / f"{item.stem}_{timestamp()}{item.suffix}"
            )
...
def validate_project() -> bool:
    print("\n[INFO] Validando sintaxe do projeto...")
...
def main() -> int:
    ...
    # Correção principal:
    # SECRETS_EXAMPLE_PATH é um Path e
    # SECRETS_EXAMPLE_CONTENT é uma string.
    if not SECRETS_EXAMPLE_PATH.exists():
        write_file(
            SECRETS_EXAMPLE_PATH,
            SECRETS_EXAMPLE_CONTENT,
        )
    else:
        print(
            f"[OK] Mantido: {SECRETS_EXAMPLE_PATH}"
        )
    ...
    print("\n[CONCLUÍDO]")
    print("Configure as credenciais em:")
    print("  .streamlit/secrets.toml")
    print()
    print("Para gerar o hash da senha, execute:")
    print(
        "  python -c "
        "\"import hashlib; "
        "print(hashlib.sha256("
        "'SuaSenhaForte123!'.encode()).hexdigest())\""
    )
    print()
    print("Depois inicie o Streamlit:")
